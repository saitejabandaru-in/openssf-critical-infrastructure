...
async function compareRepositories() {
    ...
    try {
        ...
        // Render Dual Radar overlay
        if (radarChartInstance) {
            radarChartInstance.data.datasets[0].label = repoA;
            radarChartInstance.data.datasets[1].label = repoB;
            radarChartInstance.data.datasets[1].borderDash = [];
            radarChartInstance.data.datasets[1].borderColor = '#c084fc';
            radarChartInstance.data.datasets[1].backgroundColor = 'rgba(192, 132, 252, 0.2)';
            radarChartInstance.update();
        }
    } catch (err) {
...
function downloadAuditReport() {
    ...
    const content = `# OpenSSF Criticality Audit Report: ${currentRepoName}\n\n` +
        ...
        `\n\n---\nReport generated by OpenSSF Criticality Suite v2.2 Enterprise Flagship`;
...
function downloadJsonReport() {
    ...
    const payload = {
        ...
        status: score >= 0.400 ? "Critical Infrastructure" : "Standard Project",
    ...
    const a = document.createElement('a');
    a.href = url;
    a.download = `OpenSSF_Criticality_${currentRepoName.replace('/', '_')}.json`;
    a.click();
    URL.revokeObjectURL(url);
}
